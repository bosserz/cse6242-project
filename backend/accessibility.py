...
# Data
walking_graph = nx.read_graphml('../data/all_data/modified_london_walking_graph.graphml')
subway_graph = nx.read_graphml('../data/all_data/london_rail_network.graphml')
centroid_points = gpd.read_file("../data/all_data/neighborhood_centroid.geojson")
poi_gdf = gpd.read_file("../data/all_data/projected_osm_pois_point.geojson")

def open_geojson_as_json(filepath):
    with open(filepath, 'r') as f:
        data = json.load(f)
    return data

geojson_data = open_geojson_as_json("../notebooks/london_neighborhood.geojson")

with open("../data/all_data/subway_walk_node_mapper.json", "r") as json_file:
    subway_walk_node_mapper = json.load(json_file)

with open("../data/all_data/centroid_station_travel_time.json", "r") as json_file:
...
def run_acc(poi_gdf,
    ...
    for _, row in centroid_points.iterrows():
        ...
        # df_reachable_poi_agg["centroid_id"] = centroid_id
